...
def sql_schedule(
    ...
    teams_sql = ""
    for team in teams:
        if team not in state.team_uuids:
            teams_sql += f"('{state.team_uuids.setdefault(team, str(uuid.uuid4()))}', '{team}'),\n"
    if teams_sql != "":
