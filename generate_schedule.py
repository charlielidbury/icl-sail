...
def sql_schedule(
    ...
    for race in races:
        ...
        races_sql += f"('{state.race_uuids[race]}', {race_num_offset + race.race_num}, '{league}'),\n"
    if races_sql != "":
...
if __name__ == "__main__":
    ...
    if True:
        ...
        gold_schedule = greedy_blocks(TEAMS[:11], FLIGHTS[:2])
        silver_schedule = greedy_blocks(TEAMS[11:], FLIGHTS[2:])
        schedule = []
        i = 0
        while i < len(gold_schedule) or i < len(silver_schedule):
