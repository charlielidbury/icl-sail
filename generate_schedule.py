...
# 4 flights
FLIGHTS = [
    (("Pink Stripe", (7, 8, 9)), ("Black Stripe", (10, 11, 12))),
    (("Green Circle", (7, 8, 9)), ("Black Diamond", (10, 11, 12))),
    (
        ("Red Striped Jibs", (1, 2, 3)),
        ("Blue Striped Jibs", (4, 5, 6)),
    ),
    (("Red Jib", (1, 2, 3)), ("Blue Jib", (4, 5, 6))),
]
...
def sql_state(state: StateManager, teams: List[str], flights: List[Flight]) -> str:
    ...
    for left_flight, right_flight in flights:
        flights_sql += f"('{state.flight_uuids.setdefault(left_flight, str(uuid.uuid4()))}', '{left_flight[0]}', ARRAY[{','.join(map(str, left_flight[1]))}]),\n"
        flights_sql += f"('{state.flight_uuids.setdefault(right_flight, str(uuid.uuid4()))}', '{right_flight[0]}', ARRAY[{','.join(map(str, right_flight[1]))}]),\n"
    ...
    for team in teams:
        teams_sql += (
            f"('{state.team_uuids.setdefault(team, str(uuid.uuid4()))}', '{team}'),\n"
        )
...
def sql_schedule(
    ...
    # for races that were generated by this script previously
    state.races_output += len(races)
    sql = ""

    # insert races
    races_sql = ""
    for race in races:
        state.race_uuids[race] = str(uuid.uuid4())  # always override
        races_sql += (
            f"('{state.race_uuids[race]}', {race.race_num}, '{race.league}'),\n"
        )
    if races_sql != "":
    ...
    for race in races:
        ...
        raceteams_sql += f"('{state.race_uuids[race]}', '{state.team_uuids[race.team1]}', '{state.flight_uuids[left_flight]}'),\n"
        raceteams_sql += f"('{state.race_uuids[race]}', '{state.team_uuids[race.team2]}', '{state.flight_uuids[right_flight]}'),\n"
    if raceteams_sql != "":
...
if __name__ == "__main__":
    ...
    sql_header = sql_state(state, TEAMS, FLIGHTS)
    ...
    if False:
        ...
        state.races_output = 0
        for r in schedule:
            r.league = league
        with open(f"schedule/{league}.tsv", "w") as f:
            f.write(tsv_schedule(teams, flights, schedule))
        with open(f"schedule/{league}.sql", "w") as f:
            f.write(sql_schedule(teams, flights, schedule, league, state))
    else:
        state.races_output = 190
        race_num = 191
        gold_schedule = greedy_blocks(TEAMS[:10], FLIGHTS[:2])
        for r in gold_schedule:
            r.league = "semis/gold"
        silver_schedule = greedy_blocks(TEAMS[10:], FLIGHTS[2:])
        for r in silver_schedule:
            r.league = "semis/silver"
        schedule = []
        while len(gold_schedule) > 0 or len(silver_schedule) > 0:
            # gold, gold, silver, silver, gold, gold, silver, silver, ...
            for s in [gold_schedule, silver_schedule]:
                rs, s[:] = s[:2], s[2:]
                for r in rs:
                    r.race_num = race_num
                    race_num += 1
        ...
        with open(f"schedule/semis.tsv", "w") as f:
            f.write(tsv_schedule(TEAMS, FLIGHTS, schedule))
        with open(f"schedule/semis.sql", "w") as f:
            f.write(sql_schedule(TEAMS, FLIGHTS, schedule, "semis", state))
